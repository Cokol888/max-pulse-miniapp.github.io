        ...
        <Grid>
          ...
          <Grid>
            ...
            {validation && validation.skipped && (
              <Typography.Body variant="small">Skipped (not in MAX)</Typography.Body>
            )}
            {validation && validation.ok && (
              <Typography.Body variant="small">
                ✅ Validated: user=
                {String((validation.data?.user as { id?: number; username?: string })?.id ?? '-')}{' '}
                {String((validation.data?.user as { username?: string })?.username ?? '')} start_param=
                {validation.data?.start_param ?? '-'}
              </Typography.Body>
            )}
            {validation && !validation.ok && !validation.skipped && (
              <Typography.Body variant="small">❌ Not validated: {validation.reason}</Typography.Body>
            )}
          </Grid>
